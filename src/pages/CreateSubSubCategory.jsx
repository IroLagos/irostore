...
const CreateSubSubCategory = () => {
  const navigate = useNavigate()
    ...
    const res = await axios.post(`${URL}/api/subsubs/create`, {name}, {
      headers:{
        'Content-Type':'application/json'
      }
    });
    if (res){
      navigate('/subsubcategorytable')
    }
  }
 ...
 <div className="flex flex-col gap-y-6">
        <p className="text-2xl text-center">Create Sub Sub-Category</p>
          <input onChange={(e)=>setName(e.target.value)} className="border border-black px-2 py-1 w-full md:w-[500px]" placeholder="Name" />



<button onClick={handleSubCategory} className="bg-blue-500 text-white py-1">{isLoading ? "Creating ..." : "Create Sub Sub-Category"}</button>
