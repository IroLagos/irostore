...
const BrandTable = () => {
  ...
  const handleDelete = async(itemId)=>{
    try{
      const res = await axios.delete(`${URL}/api/brands/${itemId}`)
  ...
  return (
    <div>
        ...
        <div className='p-9'>
        <p className='font-bold text-3xl text-[#5b3e31]'>Brands</p>
      <div className="mb-4 text-right mt-4">
        ...
        <Link to={'/createbrand'}><button className='bg-[#5b3e31] text-white px-6 py-2 rounded-md'>Create Brand</button></Link>
      </div>
        ...
        <tbody>
          ...
          {categories.map((p, index) => (
            <tr key={p.id} className=' border-b border-gray-300'>
              ...
              <Link to={`/editbrand/${p.id}`}><td className="py-4 px-11 pt-[25px]"><GrEdit /></td></Link>
