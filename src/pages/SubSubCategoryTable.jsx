...
const SubSubCategoryTable = () => {
  ...
  const fetchCategories = async () => {
    const res = await axios.get(`${URL}/api/subsubs`)
  ...
  const handleDelete = async(itemId)=>{
    try{
      ...
      setSubCategories((prevData) => prevData.filter(item => item.id !== itemId));
      console.log(res.data)
    }
    catch(err){
      console.log(err)
  ...
  return (
    <div>
        ...
        <div className='p-9'>
        <p className='font-bold text-3xl text-[#5b3e31]'>Sub Sub-Categories</p>
      <div className="mb-4 text-right mt-4">
        ...
        <Link to={'/createsubsubcategory'}><button className='bg-[#5b3e31] text-white px-6 py-2 rounded-md'>Create Sub Sub-Category</button></Link>
      </div>
