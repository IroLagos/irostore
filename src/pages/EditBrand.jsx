...
//   const [selectedCategoryId, setSelectedCategoryId] = useState("")
  ...
  const fetchCategories = async () => {
    try {
      const res = await axios.get(`${URL}/api/brands/${categoryId}`)
  ...
  const editBrand = async (e) => {
    ...
    try {
      const res = await axios.put(`${URL}/api/brands/${categoryId}`, updatedCategory, {
        headers: {
      ...
      <div className="items-center h-[100vh] gap-x-9 justify-center flex w-full">
        <form onSubmit={editBrand}>
          <div className="flex flex-col gap-y-6">
            <p className="text-2xl text-center">Edit Brand</p>
            ...
            <button type="submit" disabled={isLoading} className="bg-blue-500 text-white py-1">{isLoading ? "Updating ..." : "Update brand name"}</button>
          </div>
        </form>
